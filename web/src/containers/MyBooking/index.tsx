        ...
        <PageContainer icon={<IoBookOutline />} title="My Booking">
            <div className="tw-flex tw-flex-col tw-gap-5">
                <div className="tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-3">
                    <div className="tw-flex tw-items-center tw-gap-1 tw-px-1 tw-py-1 tw-rounded tw-bg-pink-50 tw-border-pink-100 tw-border">
                        <button disabled={index === 0} onClick={goBack} className="disabled:tw-bg-slate-200 tw-p-1 tw-shadow-lg tw-bg-pink-500 tw-text-white tw-rounded"><FiChevronLeft size="26" /></button>
                        {period ? (
                            <span className="tw-flex tw-items-center tw-gap-3 tw-text-pink-500 tw-p-2 tw-rounded">{dayjs(period?.from).format('DD MMM YYYY')} - {dayjs(period?.to).format('DD MMM YYYY')}</span>
                        ) : (
                            <span className="tw-text-pink-500 tw-bg-pink-50 tw-p-2 tw-rounded">Period is not opened. Go back current period</span>
                        )}
                        <button disabled={index === periods.length - 1} onClick={goNext} className="disabled:tw-bg-slate-200 tw-p-1 tw-shadow-lg tw-bg-pink-500 tw-text-white tw-rounded"><FiChevronRight size="26" /></button>
                    </div>
                </div>
                {period && (
                                    ...
                                    return (
                                        <th key={i} className="tw-h-10 tw-justify-center tw-items-center">
                                            <div className="tw-flex tw-flex-col">
                                                <span className="tw-text-pink-500 tw-font-bold tw-uppercase">{dayjs(d).format('ddd')}</span>
                                                <small className="tw-text-pink-500/50">{dayjs(d).format('DD-MMM-YYYY')}</small>
                                            </div>
                                        </th>
                                    )
                                })}
                                    ...
                                    return (
                                        <td key={i}>
                                            {period.isCurrentPeriod ? (
                                                ...
                                                <div className="tw-h-16 tw-flex tw-items-center tw-justify-center tw-transition-all">
                                                    <span className={cn({ "tw-text-pink-500": booking, "tw-text-slate-200": !booking })}><IoLockClosedSharp size={40} /></span>
                                                </div>
                                            )}
                                        </td>
                                    )
